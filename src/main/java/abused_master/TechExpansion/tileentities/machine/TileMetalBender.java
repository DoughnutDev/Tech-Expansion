...
public class TileMetalBender extends TileEntity implements ITickable, ISidedInventory, IEnergyProvider, IEnergyReceiver {
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        for(int i = 0; i < inventory.tagCount(); i++) {
            ...
            metalbendInv.set(itemTag.getByte("Slot"), stack);
        }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
        ...
        for(byte slot = 0; slot < metalbendInv.size(); slot++) {
            ItemStack stack = metalbendInv.get(slot);
    ...
    public int getSizeInventory() {
        return this.metalbendInv.size();
    }


    @Override
    public ItemStack getStackInSlot(int index) {
        return (ItemStack)this.metalbendInv.get(index);
    }


    @Override
    public ItemStack decrStackSize(int index, int count) {
        return ItemStackHelper.getAndSplit(this.metalbendInv, index, count);
    }


    @Override
    public ItemStack removeStackFromSlot(int index) {
        return ItemStackHelper.getAndRemove(this.metalbendInv, index);
    }
    ...
    @Override
    public void clear() {
        this.metalbendInv.clear();
    }


    @Override
    public String getName() {
        return ModBlocks.MetalBender.getUnlocalizedName();
    }
    ...
    {
        if (((ItemStack)this.metalbendInv.get(0)).func_190926_b()) {
            return false;
        }else {
            return true;
        }
    ...
    public void update() {
        if (this.canBend()) {
            ItemStack itemstack = (ItemStack) this.metalbendInv.get(0);
            ItemStack itemstack1 = RecipeMetalBender.instance().getBendingResult(itemstack);
            ItemStack itemstack2 = (ItemStack) this.metalbendInv.get(1);
            if (storage.getEnergyStored() >= 50 && !itemstack1.func_190926_b()) {
                storage.setEnergyStored(storage.getEnergyStored() - 50);

                if (itemstack2.func_190926_b()) {
                    this.metalbendInv.set(1, itemstack1.copy());
    ...
    public boolean func_191420_l() {
        for (ItemStack itemstack : this.metalbendInv)
        {
